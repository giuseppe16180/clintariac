...
public class ContextManager {
    ...
    private void initEmailHandler() {
        ...
        final Predicate<EmailData> isNotValidSubject = email -> {
            ...
            return Stream.of(comunToken, deleteToken, confirmToken).noneMatch(token -> token.equals(subject));
        };
        ...
        final Predicate<EmailData> isNewMessage = email -> email.subject.toLowerCase().equals(comunToken);
        ...
        final Consumer<EmailData> consumeNewMessage = email -> {
            ...
            dataManager.setUser(new UserData(user.firstName, user.lastName, user.id, user.email, user.phone, chat)); // todo:
                                                                                                                     // cosa
                                                                                                                     // migliore
                                                                                                                     // nel
                                                                                                                     // datamangarer
            ...
            Optional<TicketData> oldWaitingTicket = dataManager.getTicketsList().stream()
                    .filter(ticket -> ticket.user.equals(user.id) && ticket.state == TicketState.AWAITING).findFirst();
            ...
            dataManager.setTicket(new TicketData(newTicketId, user.id, TicketState.AWAITING, LocalDateTime.MAX,
    ...
    private boolean addToChat(String userId, String message, boolean isUserSent) {
        ...
        if (user.isPresent()) {
            ...
            dataManager.setUser(new UserData(user.get().firstName, user.get().lastName, user.get().id, user.get().email,
                    user.get().phone, chat));
        }
    ...
    public void setTicket(TicketData newTicket) {
        ...
        if (isSent) {
            addToChat(newTicket.user, String.format("Proposta appuntamento - %s %s", dateTime[0], dateTime[1]), false);
    ...
    public List<TicketData> getReservationsStartToDate(LocalDate date) {
        return dataManager.getTicketsList().stream().filter(ticket -> {
            return (ticket.state == TicketState.CONFIRMED || ticket.state == TicketState.BOOKED)
                    && (ticket.booking.toLocalDate().equals(date) || ticket.booking.toLocalDate().isAfter(date));
