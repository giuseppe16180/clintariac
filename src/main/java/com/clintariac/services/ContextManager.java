...
public class ContextManager {
    ...
    private void initEmailHandler() {
        ...
        final Predicate<EmailData> isNotValidSubject = email -> {
            ...
            return Stream.of(newToken, deleteToken, confirmToken)
        ...
        final Predicate<EmailData> isNewTicket =
                email -> email.subject.toLowerCase().equals(newToken);
        ...
        final Consumer<EmailData> consumeNewTicket = email -> {
            ...
            dataManager.setTicket(
                    new TicketData(newTicketId, userId, TicketState.AWAITING, LocalDateTime.MAX,
    ...
    public boolean sendMessage(String userId, String message) {
        ...
        if (isSent) {
            UserData user = dataManager.getUser(userId).get();
            List<MessageData> chat = user.getChat();
            chat.add(new MessageData(message, LocalDateTime.now(), false));
