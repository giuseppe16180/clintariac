...
public class ContextManager {
    ...
    private void initEmailHandler() {
        ...
        final Predicate<EmailData> isNotValidSubject = email -> {
            ...
            return Stream.of(comunToken, deleteToken, confirmToken)
                    .noneMatch(token -> token.equals(subject));
        };
        ...
        final Predicate<EmailData> isNewMessage =
                email -> email.subject.toLowerCase().equals(comunToken);
        ...
        final Consumer<EmailData> consumeNewMessage = email -> {
            ...
            dataManager.setUser(new UserData(
                    ...
                    user.email,
                    user.phone,
                    chat)); // todo: cosa migliore nel datamangarer
            ...
            dataManager.setTicket(new TicketData(newTicketId, user.id, TicketState.AWAITING,
                    LocalDateTime.MAX,
    ...
    // TODO: non va fatto cosi', il context mangaer dovrebbe avere un metodo per fare ci`o per cui
    // serve sto coso dal context manager stesso, vedi l'uso che si fa del metodo nel
    // dashboardcontroller
